...
def handle_text(message):
    try:
        ...
        if saved_reply:
            ...
            # AI Generation with Error Handling (Fix 1)
            ...
            try:
                ...
                error_str = str(e)
                if "429" in error_str:
                    ai_reply = "😴 **Quota Exceeded:** Raj bhai, mera aaj ka limit khatam ho gaya. Please kal try karein! (Error 429)"
                    source = "System - Quota"
                else:
                    ai_reply = f"❌ **System Error:** Kuch gadbad hai.\n`{error_str}`"
                    source = "System - Error"
                    print(f"Gemini API Error: {e}")

        # Send to Telegram (Formatting Fix - Fix 2)
        ...
        try:
            ...
            # If Markdown fails (Bad Request), send as plain text
            if "can't parse entities" in str(e):
                print(f"Markdown Error ignored, sending plain text.")
                bot.reply_to(message, ai_reply, parse_mode=None, reply_markup=markup)
            else:
...
# --- RUN ---
def run_bot():
    print("🤖 Bot Started (Fixed TTS + Quota Handling)...")
